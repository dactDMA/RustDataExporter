#pragma once

#include <array>
#include <cstdint>
#include <string_view>
#include <unordered_map>

// Generated by RustDataExporter from ItemManager.itemList
// Generated UTC: 2026-08-06 18:36:45
// Target build: Protocol 2632.287.1 / Changeset 160450

namespace rust_items
{
    struct ItemEntry
    {
        std::int32_t item_id;
        std::string_view shortname;
    };

    inline constexpr std::array<ItemEntry, 1252> ITEMS = {{
        { -866121090, "2module.car" },
        { -226151558, "2module.car.chassis" },
        { -831725027, "3module.car" },
        { 1482871705, "3module.car.chassis" },
        { -935322684, "4module.car" },
        { 385099196, "4module.car.chassis" },
        { 162882477, "50cal.mounted" },
        { -1467876094, "50cal.mounted.left" },
        { 1248383659, "50cal.mounted.right" },
        { 1840570710, "abovegroundpool" },
        { -880494890, "abyss.barrel.horizontal" },
        { -919882824, "abyss.barrel.vertical" },
        { -1896395719, "advancedblueprintfragment" },
        { -1498613415, "advancedcoolingtea" },
        { -652889722, "advancedcraftingtea_quality" },
        { -334418777, "advancedwarmingtea" },
        { -1385721419, "advanceharvestingtea" },
        { 343045591, "aiming.module.mlrs" },
        { 1055319033, "ammo.grenadelauncher.buckshot" },
        { 349762871, "ammo.grenadelauncher.he" },
        { 915408809, "ammo.grenadelauncher.smoke" },
        { 588596902, "ammo.handmade.shell" },
        { 963150711, "ammo.mortar.basic" },
        { -486432631, "ammo.mortar.fragment" },
        { -2097376851, "ammo.nailgun.nails" },
        { 385645417, "ammo.paintball" },
        { 785728077, "ammo.pistol" },
        { 51984655, "ammo.pistol.fire" },
        { -1691396643, "ammo.pistol.hv" },
        { -1211166256, "ammo.rifle" },
        { -1321651331, "ammo.rifle.explosive" },
        { 1712070256, "ammo.rifle.hv" },
        { 605467368, "ammo.rifle.incendiary" },
        { -742865266, "ammo.rocket.basic" },
        { 1638322904, "ammo.rocket.fire" },
        { -1841918730, "ammo.rocket.hv" },
        { -1843426638, "ammo.rocket.mlrs" },
        { -384243979, "ammo.rocket.sam" },
        { 1296788329, "ammo.rocket.seeker" },
        { -17123659, "ammo.rocket.smoke" },
        { -1685290200, "ammo.shotgun" },
        { -1036635990, "ammo.shotgun.fire" },
        { -727717969, "ammo.shotgun.slug" },
        { 550753330, "ammo.snowballgun" },
        { 829641693, "anchor" },
        { -1432674913, "antiradpills" },
        { 1664052604, "apartment.master_key" },
        { 1548091822, "apple" },
        { 352130972, "apple.spoiled" },
        { 359723196, "arcade.machine.chippy" },
        { 215754713, "arrow.bone" },
        { 14241751, "arrow.fire" },
        { -1023065463, "arrow.hv" },
        { -1234735557, "arrow.wooden" },
        { 1113514903, "attackhelicopter" },
        { -1622110948, "attire.banditguard" },
        { -1266045928, "attire.bunny.onesie" },
        { -1004426654, "attire.bunnyears" },
        { -747743875, "attire.egg.suit" },
        { 794356786, "attire.hide.boots" },
        { 3222790, "attire.hide.helterneck" },
        { 1722154847, "attire.hide.pants" },
        { 980333378, "attire.hide.poncho" },
        { -1773144852, "attire.hide.skirt" },
        { 196700171, "attire.hide.vest" },
        { 1081315464, "attire.nesthat" },
        { -1506417026, "attire.ninja.suit" },
        { -324675402, "attire.reindeer.headband" },
        { -842267147, "attire.snowman.helmet" },
        { -2139580305, "autoturret" },
        { -262590403, "axe.salvaged" },
        { 405904531, "ball" },
        { -19318653, "ballista.bolt.hammerhead" },
        { -1987565603, "ballista.bolt.incendiary" },
        { -1127003365, "ballista.bolt.piercer" },
        { -357442017, "ballista.bolt.pitchfork" },
        { -759279626, "ballista.mounted" },
        { 1714509152, "ballista.static" },
        { 1127732084, "ballistic.helmet" },
        { 1983541158, "ballistic.legarmor" },
        { -1780402255, "ballistic.vest" },
        { -1652561344, "bamboo.barrel" },
        { -2072273936, "bandage" },
        { -1639742441, "bar.stool.metal" },
        { -1444366079, "bar.stool.wood" },
        { -1215166612, "barrelcostume" },
        { -1950721390, "barricade.concrete" },
        { -424687710, "barricade.medieval" },
        { 1655650836, "barricade.metal" },
        { -559599960, "barricade.sandbags" },
        { 15388698, "barricade.stone" },
        { 866889860, "barricade.wood" },
        { 1373240771, "barricade.wood.cover" },
        { 1382263453, "barricade.woodwire" },
        { -1024954624, "base.half.shelves" },
        { 655356057, "base.horizontal.barrel" },
        { -1695149731, "base.single.shelves" },
        { 2045583965, "base.vertical.barrel" },
        { -143481979, "basicblueprintfragment" },
        { -1274093662, "bathtub.planter" },
        { -187304968, "batteringram" },
        { -479314201, "batteringram.head.repair" },
        { 609049394, "battery.small" },
        { 1099314009, "bbq" },
        { 1468749025, "bdu.pants" },
        { -2127311451, "bdu.shirt" },
        { -321431890, "beachchair" },
        { -1621539785, "beachparasol" },
        { 657352755, "beachtable" },
        { -8312704, "beachtowel" },
        { -576866254, "beanbagseatfabric" },
        { -1220928936, "beanbagseatleather" },
        { -1520560807, "bearmeat" },
        { -989755543, "bearmeat.burned" },
        { 1873897110, "bearmeat.cooked" },
        { 1348294923, "bearmeat.spoiled" },
        { -1273339005, "bed" },
        { 184516676, "beehive" },
        { 821588319, "bicycle" },
        { -2095813057, "bigcatmeat" },
        { -1318837358, "bigcatmeat.cooked" },
        { 248643189, "bigcatmeat.spoiled" },
        { 1771755747, "black.berry" },
        { 1931713481, "black.raspberries" },
        { 1553078977, "bleach" },
        { 1776460938, "blood" },
        { -851288382, "blowpipe" },
        { 1112162468, "blue.berry" },
        { -586342290, "blueberries" },
        { 1036321299, "bluedogtags" },
        { 1364514421, "blueidtag" },
        { -996920608, "blueprintbase" },
        { -880412831, "blunderbuss" },
        { -321247698, "boat.planner" },
        { 1993693904, "boatbuildingstation" },
        { 1746956556, "bone.armor.suit" },
        { 1711033574, "bone.club" },
        { 1719978075, "bone.fragments" },
        { -1478094705, "boogieboard" },
        { -1113501606, "boombox" },
        { 1680793490, "boomerang" },
        { -1000573653, "boots.frog" },
        { 613961768, "botabag" },
        { 755224797, "bottle.vodka" },
        { 884424049, "bow.compound" },
        { 1443579727, "bow.hunting" },
        { 803222026, "box.repair.bench" },
        { -180129657, "box.wooden" },
        { 833533164, "box.wooden.large" },
        { 281099360, "bread.loaf" },
        { 850280505, "bucket.helmet" },
        { 1424075905, "bucket.water" },
        { 1525520776, "building.planner" },
        { 1285226495, "bunny.suit" },
        { 1366282552, "burlap.gloves" },
        { 21402876, "burlap.gloves.new" },
        { 1877339384, "burlap.headwrap" },
        { 602741290, "burlap.shirt" },
        { -761829530, "burlap.shoes" },
        { 1992974553, "burlap.trousers" },
        { 1732236518, "caboose" },
        { 1783512007, "cactusflesh" },
        { 1973165031, "cakefiveyear" },
        { 1946219319, "campfire" },
        { -700591459, "can.beans" },
        { 1655979682, "can.beans.empty" },
        { -1941646328, "can.tuna" },
        { -1557377697, "can.tuna.empty" },
        { 1121925526, "candycane" },
        { 1789825282, "candycaneclub" },
        { -34498533, "cannon" },
        { -411735114, "cannonball" },
        { 1230691307, "captainslog" },
        { 656371028, "carburetor1" },
        { 656371027, "carburetor2" },
        { 656371026, "carburetor3" },
        { 1081921512, "cardtable" },
        { 1524980732, "carvable.pumpkin" },
        { 476066818, "cassette" },
        { -912398867, "cassette.medium" },
        { 1523403414, "cassette.short" },
        { 1145722690, "catapult" },
        { 1954597876, "catapult.ammo.bee" },
        { 1831249347, "catapult.ammo.boulder" },
        { -1827561369, "catapult.ammo.explosive" },
        { -484006286, "catapult.ammo.incendiary" },
        { 634478325, "cctv.camera" },
        { 1142993169, "ceilinglight" },
        { 1104520648, "chainsaw" },
        { 1534542921, "chair" },
        { -463012608, "chair.ejector.seat" },
        { -1944704288, "chair.icethrone" },
        { -1938052175, "charcoal" },
        { 2130820932, "charity.plushy.01" },
        { 2130820933, "charity.plushy.02" },
        { 2130820934, "charity.plushy.03" },
        { 2130820927, "charity.plushy.04" },
        { 1973684065, "chicken.burned" },
        { -1848736516, "chicken.cooked" },
        { -152332823, "chicken.costume" },
        { -1440987069, "chicken.raw" },
        { -751151717, "chicken.spoiled" },
        { -2018158920, "chickencoop" },
        { -1916473915, "chineselantern" },
        { -770304148, "chineselanternwhite" },
        { -965336208, "chocolate" },
        { -105343718, "circleballoon2025" },
        { 486661382, "clantable" },
        { 968019378, "clatter.helmet" },
        { 122783240, "clone.black.berry" },
        { 838831151, "clone.blue.berry" },
        { -778875547, "clone.corn" },
        { -1305326964, "clone.green.berry" },
        { -886280491, "clone.hemp" },
        { -798662404, "clone.orchid" },
        { 1512054436, "clone.potato" },
        { 1898094925, "clone.pumpkin" },
        { 2133269020, "clone.red.berry" },
        { -19360132, "clone.rose" },
        { 912235912, "clone.sunflower" },
        { 924598634, "clone.wheat" },
        { 1533551194, "clone.white.berry" },
        { 390728933, "clone.yellow.berry" },
        { -858312878, "cloth" },
        { -606898372, "clothing.mannequin" },
        { -903796529, "clothing.mod.armorinsert_asbestos" },
        { 2047789913, "clothing.mod.armorinsert_lead" },
        { 1099611828, "clothing.mod.armorinsert_metal" },
        { -593892112, "clothing.mod.armorinsert_wood" },
        { 204391461, "coal" },
        { 1873004466, "cocoknight.armor.gloves" },
        { -582467439, "cocoknight.armor.helmet" },
        { 507284030, "cocoknight.armor.pants" },
        { 1426097945, "cocoknight.armor.torso" },
        { -24571537, "coconut" },
        { -803263829, "coffeecan.helmet" },
        { 573676040, "coffin.storage" },
        { -1247485104, "command.block" },
        { 594041190, "compass" },
        { -1593678393, "component.box.ammo.large" },
        { 1254295946, "component.box.armor.large" },
        { 1884461210, "component.box.charcoal.large" },
        { 1736620421, "component.box.clothing.large" },
        { -413663149, "component.box.comps.large" },
        { -1998423571, "component.box.explosives.large" },
        { 1023919015, "component.box.food.large" },
        { -544295594, "component.box.guns.large" },
        { -800824218, "component.box.meds.large" },
        { 1465782238, "component.box.metal.large" },
        { 992944937, "component.box.ore.large" },
        { 574701440, "component.box.scrap.large" },
        { 94971664, "component.box.stone.large" },
        { -10594280, "component.box.sulfur.large" },
        { 679690962, "component.box.tools.large" },
        { 1044081720, "component.box.wood.large" },
        { -1488398114, "composter" },
        { -1588628467, "computerstation" },
        { 1176355476, "concretehatchet" },
        { -1360171080, "concretepickaxe" },
        { 1603174987, "confetticannon" },
        { 968421290, "connected.speaker" },
        { 1456143403, "cookingworkbench" },
        { -1260229965, "coolingtea" },
        { 1367190888, "corn" },
        { 368008432, "craftingtea_quality" },
        { 1158340334, "crankshaft1" },
        { 1158340331, "crankshaft2" },
        { 1158340332, "crankshaft3" },
        { 1189981699, "cratecostume" },
        { -1081599445, "crocodilemeat" },
        { 392828520, "crocodilemeat.cooked" },
        { -1796837031, "crocodilemeat.spoiled" },
        { 1965232394, "crossbow" },
        { 2022157467, "crossbowbowless" },
        { -321733511, "crude.oil" },
        { -97956382, "cupboard.tool" },
        { 1488606552, "cupboard.tool.retro" },
        { 1174957864, "cupboard.tool.shockbyte" },
        { 1242522330, "cursedcauldron" },
        { -963819285, "dart.incapacitate" },
        { -594596146, "dart.radiation" },
        { 2036395619, "dart.scatter" },
        { -274709858, "dart.wood" },
        { -872679147, "dartboard" },
        { -1903165497, "deer.skull.mask" },
        { -78533081, "deermeat.burned" },
        { -1509851560, "deermeat.cooked" },
        { 1422530437, "deermeat.raw" },
        { -2035449523, "deermeat.spoiled" },
        { 1568388703, "diesel_barrel" },
        { 1895235349, "discoball" },
        { 286648290, "discofloor" },
        { 1735402444, "discofloor.largetiles" },
        { -1800102806, "discord.plushie" },
        { 1494014226, "discord.trophy" },
        { 1046904719, "diverhatchet" },
        { 1561022037, "diverpickaxe" },
        { 1846605708, "divertorch" },
        { 296519935, "diving.fins" },
        { -113413047, "diving.mask" },
        { -2022172587, "diving.tank" },
        { -1559420426, "diving.tank.double" },
        { -1101924344, "diving.wetsuit" },
        { -451310088, "documents" },
        { 1223900335, "dogtagneutral" },
        { 1409529282, "door.closer" },
        { -1151332840, "door.double.hinged.bardoors" },
        { 1390353317, "door.double.hinged.metal" },
        { 1221063409, "door.double.hinged.toptier" },
        { -1336109173, "door.double.hinged.wood" },
        { -1063073030, "door.hinged.boat.wood" },
        { 818733919, "door.hinged.industrial.a" },
        { -2067472972, "door.hinged.metal" },
        { 1353298668, "door.hinged.toptier" },
        { 1729120840, "door.hinged.wood" },
        { -1112793865, "door.key" },
        { -258574361, "draculacape" },
        { 1865253052, "draculamask" },
        { -854270928, "dragondoorknocker" },
        { 1588492232, "drone" },
        { -1519126340, "dropbox" },
        { -1330640246, "drumkit" },
        { 1401987718, "ducttape" },
        { -1779203452, "easel" },
        { 844440409, "easter.bronzeegg" },
        { -1002156085, "easter.goldegg" },
        { -126305173, "easter.paintedeggs" },
        { 1757265204, "easter.silveregg" },
        { 1856217390, "easterbasket" },
        { -979302481, "easterdoorwreath" },
        { 1858828593, "egg" },
        { 1171735914, "electric.andswitch" },
        { 2100007442, "electric.audioalarm" },
        { 553270375, "electric.battery.rechargable.large" },
        { 2023888403, "electric.battery.rechargable.medium" },
        { -692338819, "electric.battery.rechargable.small" },
        { -690968985, "electric.blocker" },
        { 104856514, "electric.bulbstringlights" },
        { -1778897469, "electric.button" },
        { 1835946060, "electric.cabletunnel" },
        { -1510616686, "electric.chandelier" },
        { -216999575, "electric.counter" },
        { 1619039771, "electric.digitalclock" },
        { -502177121, "electric.doorcontroller" },
        { 54436981, "electric.fairylights" },
        { -939424778, "electric.flasherlight" },
        { 1892536031, "electric.fluorescentlight" },
        { 640470230, "electric.fluorescentlight.ceiling" },
        { 1849887541, "electric.fuelgenerator.small" },
        { -1196547867, "electric.furnace" },
        { -295829489, "electric.generator.small" },
        { -1507239837, "electric.hbhfsensor" },
        { -784870360, "electric.heater" },
        { -44876289, "electric.igniter" },
        { -798293154, "electric.laserdetector" },
        { -1286302544, "electric.orswitch" },
        { -2049214035, "electric.pressurepad" },
        { 492357192, "electric.random.switch" },
        { -1044468317, "electric.rf.broadcaster" },
        { 888415708, "electric.rf.receiver" },
        { -948291630, "electric.seismicsensor" },
        { -282113991, "electric.simplelight" },
        { 762289806, "electric.sirenlight" },
        { 2090395347, "electric.solarpanel.large" },
        { -563624462, "electric.splitter" },
        { -1258821205, "electric.spotlight" },
        { -2134097299, "electric.spotlight.tripod" },
        { -781014061, "electric.sprinkler" },
        { 1951603367, "electric.switch" },
        { 1717250161, "electric.tablelight" },
        { 1371909803, "electric.teslacoil" },
        { 665332906, "electric.timer" },
        { 656829501, "electric.wallcabinet" },
        { 1293102274, "electric.xorswitch" },
        { -1448252298, "electrical.branch" },
        { -458565393, "electrical.combiner" },
        { -746647361, "electrical.memorycell" },
        { 1177596584, "elevator" },
        { -1878475007, "explosive.satchel" },
        { 1248356124, "explosive.timed" },
        { -592016202, "explosives" },
        { 2054391128, "factorydoor" },
        { -1018587433, "fat.animal" },
        { -930193596, "fertilizer" },
        { -1535621066, "fireplace.stone" },
        { 1744298439, "firework.boomer.blue" },
        { 1324203999, "firework.boomer.champagne" },
        { -656349006, "firework.boomer.green" },
        { -7270019, "firework.boomer.orange" },
        { -379734527, "firework.boomer.pattern" },
        { -1553999294, "firework.boomer.red" },
        { -280223496, "firework.boomer.violet" },
        { -515830359, "firework.romancandle.blue" },
        { -1306288356, "firework.romancandle.green" },
        { -1486461488, "firework.romancandle.red" },
        { -99886070, "firework.romancandle.violet" },
        { 261913429, "firework.volcano" },
        { -454370658, "firework.volcano.red" },
        { -1538109120, "firework.volcano.violet" },
        { 342438846, "fish.anchovy" },
        { -587989372, "fish.catfish" },
        { 1668129151, "fish.cooked" },
        { -1698937385, "fish.herring" },
        { -542577259, "fish.minnows" },
        { -1904821376, "fish.orangeroughy" },
        { 989925924, "fish.raw" },
        { -851988960, "fish.salmon" },
        { -1654233406, "fish.sardine" },
        { -1768880890, "fish.smallshark" },
        { 1130729138, "fish.spoiled" },
        { -1878764039, "fish.troutsmall" },
        { 680234026, "fish.yellowperch" },
        { -1707425764, "fishing.tackle" },
        { 1569882109, "fishingrod.handmade" },
        { 559147458, "fishtrap.small" },
        { -1913996738, "fishtrophy" },
        { -1215753368, "flamethrower" },
        { 528668503, "flameturret" },
        { 304481038, "flare" },
        { -196667575, "flashlight.held" },
        { 936496778, "floor.grill" },
        { 1948067030, "floor.ladder.hatch" },
        { 607785075, "floor.ladder.hatch.toptier" },
        { 1983621560, "floor.triangle.grill" },
        { 2041899972, "floor.triangle.ladder.hatch" },
        { -478923685, "floor.triangle.ladder.hatch.toptier" },
        { -265292885, "fluid.combiner" },
        { -1166712463, "fluid.splitter" },
        { 443432036, "fluid.switch" },
        { -1973785141, "fogmachine" },
        { -1647389398, "frankensteinmask" },
        { -134959124, "frankensteins.monster.01.head" },
        { 106959911, "frankensteins.monster.01.legs" },
        { -1624770297, "frankensteins.monster.01.torso" },
        { -1732475823, "frankensteins.monster.02.head" },
        { 835042040, "frankensteins.monster.02.legs" },
        { 1491753484, "frankensteins.monster.02.torso" },
        { -297099594, "frankensteins.monster.03.head" },
        { -2024549027, "frankensteins.monster.03.legs" },
        { 1614528785, "frankensteins.monster.03.torso" },
        { 1575635062, "frankensteintable" },
        { 1413014235, "fridge" },
        { 1937380239, "frontier_hatchet" },
        { 242933621, "frontiermirror.large" },
        { 2055695285, "frontiermirror.medium" },
        { 340210699, "frontiermirror.small" },
        { 1787198294, "frontiermirror.standing" },
        { -2107018088, "fun.bass" },
        { 576509618, "fun.boomboxportable" },
        { -1530414568, "fun.casetterecorder" },
        { -1049881973, "fun.cowbell" },
        { -2040817543, "fun.flute" },
        { -2124352573, "fun.guitar" },
        { -979951147, "fun.jerrycanguitar" },
        { -1379036069, "fun.tambourine" },
        { 273172220, "fun.trumpet" },
        { 1784406797, "fun.tuba" },
        { -1999722522, "furnace" },
        { -1992717673, "furnace.large" },
        { -629028935, "fuse" },
        { -945548410, "fuse.highgrade" },
        { 352442426, "gamesroom.minifridge" },
        { 399522257, "gamesroom.shotgun.trap" },
        { -401905610, "gates.external.high.adobe" },
        { -1442339204, "gates.external.high.legacy" },
        { -691113464, "gates.external.high.stone" },
        { -335089230, "gates.external.high.wood" },
        { 479143914, "gears" },
        { 999690781, "geiger.counter" },
        { -379403794, "generator.water" },
        { -1819763926, "generator.wind.scrap" },
        { -1043618880, "ghostsheet" },
        { -695124222, "giantcandycanedecor" },
        { 282103175, "giantlollipops" },
        { -558880549, "gingerbreadsuit" },
        { -690276911, "gloweyes" },
        { -1899491405, "glue" },
        { -996235148, "goldframe.large" },
        { -1901993050, "goldframe.medium" },
        { -1836526520, "goldframe.small" },
        { -1528767189, "goldframe.standing" },
        { -1430299277, "goldframe.xl" },
        { -1322332389, "goldframe.xxl" },
        { 1365234594, "goldmirror.large" },
        { -1804515496, "goldmirror.medium" },
        { -1444650226, "goldmirror.small" },
        { 2120241887, "goldmirror.standing" },
        { -746030907, "granolabar" },
        { 809199956, "gravestone" },
        { -455286320, "grayidtag" },
        { 858486327, "green.berry" },
        { 1762167092, "greenidtag" },
        { 1840822026, "grenade.beancan" },
        { 1168916338, "grenade.bee" },
        { 143803535, "grenade.f1" },
        { -936921910, "grenade.flashbang" },
        { 1556365900, "grenade.molotov" },
        { 1263920163, "grenade.smoke" },
        { -568419968, "grub" },
        { 722955039, "gun.water" },
        { -265876753, "gunpowder" },
        { -246672609, "gunrack.horizontal" },
        { 1973949960, "gunrack.single.1.horizontal" },
        { -849373693, "gunrack.single.2.horizontal" },
        { -52398594, "gunrack.single.3.horizontal" },
        { 1132603396, "gunrack_stand" },
        { 240752557, "gunrack_tall.horizontal" },
        { -96256997, "gunrack_wide.horizontal" },
        { 352499047, "guntrap" },
        { 696029539, "hab" },
        { -1989600732, "hab.armor" },
        { -1923843855, "half.bamboo.shelves" },
        { 786458957, "halfheight.industrial.shelves" },
        { -888153050, "halloween.candy" },
        { 479292118, "halloween.lootbag.large" },
        { 1899610628, "halloween.lootbag.medium" },
        { 1319617282, "halloween.lootbag.small" },
        { 277730763, "halloween.mummysuit" },
        { -1785231475, "halloween.surgeonsuit" },
        { 200773292, "hammer" },
        { -1506397857, "hammer.salvaged" },
        { -839576748, "handcuffs" },
        { 1516531815, "harvestingtea" },
        { 1675639563, "hat.beenie" },
        { -23994173, "hat.boonie" },
        { 23391694, "hat.bunnyhat" },
        { 1714496074, "hat.candle" },
        { 1633553557, "hat.candle.birthday" },
        { -1022661119, "hat.cap" },
        { -22883916, "hat.dragonmask" },
        { 1659114910, "hat.gas.mask" },
        { -418359052, "hat.horsemask" },
        { -1539025626, "hat.miner" },
        { 1315082560, "hat.oxmask" },
        { -986782031, "hat.rabbitmask" },
        { 271048478, "hat.ratmask" },
        { -1314079879, "hat.snakemask" },
        { 709206314, "hat.tigermask" },
        { -507248640, "hat.wellipets" },
        { -1478212975, "hat.wolf" },
        { -1252059217, "hatchet" },
        { -902423513, "hazmat.krieg" },
        { 1578317134, "hazmat.plushy" },
        { 1266491000, "hazmatsuit" },
        { -470439097, "hazmatsuit.arcticsuit" },
        { -797592358, "hazmatsuit.diver" },
        { -105415879, "hazmatsuit.frontier" },
        { 972302244, "hazmatsuit.kick" },
        { 861513346, "hazmatsuit.lumberjack" },
        { 491263800, "hazmatsuit.nomadsuit" },
        { 1065594600, "hazmatsuit.pilot" },
        { -560304835, "hazmatsuit.spacesuit" },
        { -253079493, "hazmatsuit_scientist" },
        { 1107575710, "hazmatsuit_scientist_arctic" },
        { -1937799374, "hazmatsuit_scientist_naval" },
        { 86840834, "hazmatsuit_scientist_nvgm" },
        { -1958316066, "hazmatsuit_scientist_peacekeeper" },
        { 468313189, "hazmatsuittwitch" },
        { -1696379844, "hazmatyoutooz" },
        { 209218760, "head.bag" },
        { -929092070, "healingtea" },
        { -2123125470, "healingtea.advanced" },
        { -1677315902, "healingtea.pure" },
        { 362863314, "heartballoon2025" },
        { 1181207482, "heavy.plate.helmet" },
        { -1102429027, "heavy.plate.jacket" },
        { -1778159885, "heavy.plate.pants" },
        { 146221721, "heavy.scientist.plushie" },
        { -722629980, "heavyscientistyoutooz" },
        { 1160881421, "hitchtroughcombo" },
        { -1214542497, "hmlmg" },
        { -1442559428, "hobobarrel" },
        { -218009552, "homingmissile.launcher" },
        { 1601800933, "honey" },
        { -1513203236, "honeycomb" },
        { 1751045826, "hoodie" },
        { 1428574144, "hopper" },
        { 82772055, "horse" },
        { -2068194497, "horse.armor.lny26" },
        { 60528587, "horse.armor.roadsign" },
        { 1659447559, "horse.armor.wood" },
        { 1420547167, "horse.costume" },
        { -1997543660, "horse.saddle" },
        { -1323101799, "horse.saddle.double" },
        { 1559915778, "horse.saddle.single" },
        { 1400460850, "horse.saddlebag" },
        { 1989785143, "horse.shoes.advanced" },
        { -1211268013, "horse.shoes.basic" },
        { -1579932985, "horsedung" },
        { 1917703890, "horsemeat.burned" },
        { -1162759543, "horsemeat.cooked" },
        { -1130350864, "horsemeat.raw" },
        { -724146494, "horsemeat.spoiled" },
        { 363163265, "hosetool" },
        { -1982036270, "hq.metal.ore" },
        { -682687162, "humanmeat.burned" },
        { 1536610005, "humanmeat.cooked" },
        { -1709878924, "humanmeat.raw" },
        { 1272768630, "humanmeat.spoiled" },
        { 960673498, "huntingtrophylarge" },
        { -869598982, "huntingtrophysmall" },
        { -1780802565, "icepick.salvaged" },
        { 196784377, "improvised.shield" },
        { -786398324, "industrial.autoturret" },
        { 1538126328, "industrial.combiner" },
        { 610102428, "industrial.conveyor" },
        { 1430085198, "industrial.crafter" },
        { 225892284, "industrial.electric.furnace" },
        { 1868984394, "industrial.furnace.large" },
        { 346569548, "industrial.garagedoor" },
        { 742745918, "industrial.splitter" },
        { -1019111952, "industrial.storage.horizontal" },
        { -883975138, "industrial.storage.vertical" },
        { 4474927, "industrial.torch" },
        { 1623701499, "industrial.wall.light" },
        { 920930831, "industrial.wall.light.blue" },
        { 1268178466, "industrial.wall.light.green" },
        { -1160621614, "industrial.wall.light.red" },
        { -697981032, "innertube" },
        { 185586769, "innertube.horse" },
        { 2052270186, "innertube.unicorn" },
        { 210787554, "iotable" },
        { -1163532624, "jacket" },
        { -48090175, "jacket.snow" },
        { 1488979457, "jackhammer" },
        { 1242482355, "jackolantern.angry" },
        { -1824943010, "jackolantern.happy" },
        { 286193827, "jar.pickle" },
        { -1018085504, "jukebox" },
        { -97459906, "jumpsuit.suit" },
        { 1601468620, "jumpsuit.suit.blue" },
        { -874908751, "jumpsuit.waterwellnpc" },
        { 1350707894, "jungle.rock" },
        { 190184021, "kayak" },
        { -484206264, "keycard_blue" },
        { 37122747, "keycard_green" },
        { -1880870149, "keycard_red" },
        { -1622386500, "kickgems" },
        { 1814288539, "knife.bone" },
        { 158303804, "knife.bone.obsidian" },
        { -194509282, "knife.butcher" },
        { 2040726127, "knife.combat" },
        { -2073432256, "knife.skinning" },
        { -427072335, "knightsarmour.helmet" },
        { -945708533, "knightsarmour.skirt" },
        { 547862680, "knighttorso.armour" },
        { -1770281406, "krieg.chainsword" },
        { -420889602, "krieg.shotgun" },
        { 652793345, "krieg.storage.horizontal" },
        { 1305765685, "krieg.storage.vertical" },
        { -874650016, "kriegbackpack" },
        { 255305250, "ladder.wooden.boat" },
        { -316250604, "ladder.wooden.wall" },
        { 1658229558, "lantern" },
        { -907422733, "largebackpack" },
        { -489848205, "largecandles" },
        { 254522515, "largemedkit" },
        { 853471967, "laserlight" },
        { 1295301598, "latexballoon2025" },
        { -1440443161, "latexclumpballoon2025" },
        { 1223729384, "lavenderidtag" },
        { 1381010055, "leather" },
        { -73195037, "legacy bow" },
        { 607400343, "legacy.shelter.wood" },
        { -1310391395, "legacyfurnace" },
        { 242421166, "lightup.large" },
        { 1801656689, "lightup.xl" },
        { 1447138977, "lightup.xxl" },
        { -1294739579, "lightupframe.medium" },
        { 1691223771, "lightupframe.small" },
        { 1950013766, "lightupframe.standing" },
        { 450531685, "lightupmirror.large" },
        { 1028889957, "lightupmirror.medium" },
        { -389796733, "lightupmirror.small" },
        { 1916016738, "lightupmirror.standing" },
        { -2069578888, "lmg.m249" },
        { 1159991980, "lock.code" },
        { 1586884551, "lock.code.a.pilot" },
        { -850982208, "lock.key" },
        { -110921842, "locker" },
        { -2027988285, "locomotive" },
        { -1469578201, "longsword" },
        { -946369541, "lowgradefuel" },
        { -763071910, "lumberjack hoodie" },
        { -399173933, "lumberjack.hatchet" },
        { 236677901, "lumberjack.pickaxe" },
        { -1961560162, "lunar.firecrackers" },
        { 599591861, "m16a2" },
        { -1966748496, "mace" },
        { -2026042603, "mace.baseballbat" },
        { -1137865085, "machete" },
        { -586784898, "mailbox" },
        { -1035206446, "mannequin" },
        { 696029452, "map" },
        { -2012470695, "mask.balaclava" },
        { -702051347, "mask.bandana" },
        { -1184406448, "maxhealthtea" },
        { 603811464, "maxhealthtea.advanced" },
        { 1712261904, "maxhealthtea.pure" },
        { 621915341, "meat.boar" },
        { 1391703481, "meat.pork.burned" },
        { -242084766, "meat.pork.cooked" },
        { 814297925, "medieval.box.wooden.large" },
        { -380502678, "medieval.door.double.hinged.metal" },
        { -1654401345, "medieval.door.hinged.metal" },
        { -583379016, "megaphone" },
        { 1168856825, "metal.detector" },
        { -194953424, "metal.facemask" },
        { -1334569149, "metal.facemask.hockey" },
        { 110116923, "metal.facemask.icemask" },
        { 69511070, "metal.fragments" },
        { -4031221, "metal.ore" },
        { 1110385766, "metal.plate.torso" },
        { -1478855279, "metal.plate.torso.icevest" },
        { 317398316, "metal.refined" },
        { 625599716, "metal.shield" },
        { 1882709339, "metalblade" },
        { 95950017, "metalpipe" },
        { -1021495308, "metalspring" },
        { 39600618, "microphonestand" },
        { 703057617, "military flamethrower" },
        { 1361520181, "minecart.planter" },
        { 1174484438, "mini fridge" },
        { -1334255764, "minicopter" },
        { -482348853, "minicrossbow" },
        { 935606207, "minigun" },
        { 355877490, "minigunammopack" },
        { 1426574435, "minihelicopter.repair" },
        { -1130709577, "mining.pumpjack" },
        { 1052926200, "mining.quarry" },
        { 1572152877, "mintidtag" },
        { 571949408, "mixedclumpballoon2025" },
        { 1259919256, "mixingtable" },
        { -1449152644, "mlrs" },
        { -20045316, "mobilephone" },
        { 1696050067, "modularcarlift" },
        { 1459828804, "mortar.deployable" },
        { -1417478274, "motorbike" },
        { 1869224826, "motorbike_sidecar" },
        { -2047081330, "movembermoustache" },
        { 3380160, "movembermoustachecard" },
        { -648077743, "mrspice.can" },
        { -1123473824, "multiplegrenadelauncher" },
        { 809689733, "mummymask" },
        { -1962971928, "mushroom" },
        { 381595627, "neonsigntr" },
        { -961457160, "newyeargong" },
        { -1518883088, "nightvisiongoggles" },
        { 1414245162, "note" },
        { -1811234677, "nucleus" },
        { -282193997, "orangeidtag" },
        { 734320711, "orchid" },
        { 1480022580, "oretea" },
        { 2063916636, "oretea.advanced" },
        { 1729374708, "oretea.pure" },
        { -2133781216, "oubreak_scientist" },
        { 1621942085, "outbreak.sprayer" },
        { 1491189398, "paddle" },
        { -733625651, "paddlingpool" },
        { -1039234836, "paintabletarget.reactive" },
        { -707792719, "paintballgun" },
        { -1014934560, "paintballoveralls.suit" },
        { 237239288, "pants" },
        { -1695367501, "pants.shorts" },
        { -1779183908, "paper" },
        { 602628465, "parachute" },
        { 1784005657, "parachute.deployed" },
        { -575744869, "partyhat" },
        { 62577426, "photo" },
        { 1697996440, "photoframe.landscape" },
        { 1205084994, "photoframe.large" },
        { 1729712564, "photoframe.portrait" },
        { 1272430949, "piano" },
        { -1302129395, "pickaxe" },
        { 4384538, "pie.apple" },
        { 2039177180, "pie.bear" },
        { 309017792, "pie.bigcat" },
        { 120820987, "pie.chicken" },
        { 54265286, "pie.crocodile" },
        { -1785248332, "pie.fish" },
        { 320438357, "pie.hunters" },
        { 1467878256, "pie.pork" },
        { -1488408786, "pie.pumpkin" },
        { -963820355, "pie.survivors" },
        { 537946062, "pilot.hazmat.box.wooden" },
        { -1442496789, "pinata" },
        { 180752235, "pinkidtag" },
        { -144513264, "pipetool" },
        { -75944661, "pistol.eoka" },
        { -852563019, "pistol.m92" },
        { 1953903201, "pistol.nailgun" },
        { 1914691295, "pistol.prototype17" },
        { 1373971859, "pistol.python" },
        { 649912614, "pistol.revolver" },
        { 818877484, "pistol.semiauto" },
        { 1673224590, "pistol.semiauto.a.m15" },
        { -1815301988, "pistol.water" },
        { 1883981798, "piston1" },
        { 1883981801, "piston2" },
        { 1883981800, "piston3" },
        { 1090916276, "pitchfork" },
        { -952411326, "plank" },
        { 1581210395, "planter.large" },
        { 1903654061, "planter.small" },
        { -280812482, "planter.triangle" },
        { -804769727, "plantfiber" },
        { -430416124, "plantpot.single" },
        { -1651220691, "pookie.bear" },
        { -1748166144, "pooltable" },
        { 1925646349, "porkmeat.spoiled" },
        { -2086926071, "potato" },
        { -365097295, "powered.water.purifier" },
        { -892718768, "prisonerhood" },
        { -1673693549, "propanetank" },
        { 1933140008, "ptboat" },
        { 140006625, "ptz.cctv.camera" },
        { -567909622, "pumpkin" },
        { 1346158228, "pumpkinbasket" },
        { 1121416193, "purecoolingtea" },
        { 97903330, "purecraftingtea_quality" },
        { 377750553, "pureharvestingtea" },
        { -1476814093, "purewarmingtea" },
        { -1386082991, "purpleidtag" },
        { -496584751, "radiationremovetea" },
        { 2021351233, "radiationremovetea.advanced" },
        { 1905387657, "radiationremovetea.pure" },
        { -487356515, "radiationresisttea" },
        { -1729415579, "radiationresisttea.advanced" },
        { -33009419, "radiationresisttea.pure" },
        { 615112838, "rail.road.planter" },
        { 1272194103, "red.berry" },
        { -602717596, "reddogtags" },
        { 70102328, "redidtag" },
        { 969768382, "reinforced.wooden.shield" },
        { -1861522751, "research.table" },
        { -544317637, "researchpaper" },
        { -92315244, "revolver.hc" },
        { 596469572, "rf.detonator" },
        { -566907190, "rf_pager" },
        { 1394042569, "rhib" },
        { 1545779598, "rifle.ak" },
        { -139037392, "rifle.ak.diver" },
        { -1335497659, "rifle.ak.ice" },
        { 2054929933, "rifle.ak.jungle" },
        { 472505338, "rifle.ak.med" },
        { 1588298435, "rifle.bolt" },
        { -778367295, "rifle.l96" },
        { -1812555177, "rifle.lr300" },
        { 533993281, "rifle.lr300.space" },
        { 28201841, "rifle.m39" },
        { -904863145, "rifle.semiauto" },
        { -348232115, "rifle.sks" },
        { 176787552, "riflebody" },
        { 671063303, "riot.helmet" },
        { -699558439, "roadsign.gloves" },
        { -2002277461, "roadsign.jacket" },
        { 1850456855, "roadsign.kilt" },
        { 1199391518, "roadsigns" },
        { 963906841, "rock" },
        { 442886268, "rocket.launcher" },
        { -1315992997, "rocket.launcher.dragon" },
        { 494161326, "rocket.launcher.rpg7" },
        { -1863063690, "rockingchair" },
        { 1758333838, "rockingchair.rockingchair2" },
        { 192249897, "rockingchair.rockingchair3" },
        { 1414245522, "rope" },
        { 1414245519, "rose" },
        { 1878053256, "rowboat" },
        { -1985799200, "rug" },
        { -1104881824, "rug.bear" },
        { -173268129, "rustige_egg_a" },
        { -173268132, "rustige_egg_b" },
        { -173268131, "rustige_egg_c" },
        { -173268126, "rustige_egg_d" },
        { -173268125, "rustige_egg_e" },
        { -173268128, "rustige_egg_f" },
        { -173268127, "rustige_egg_g" },
        { -173268138, "rustige_egg_h" },
        { 405905095, "sail" },
        { -2110553371, "salvaged.bamboo.shelves" },
        { -1978999529, "salvaged.cleaver" },
        { -1018026008, "salvaged.industrial.shelves" },
        { 1326180354, "salvaged.sword" },
        { -1009359066, "samsite" },
        { 2126889441, "santabeard" },
        { -575483084, "santahat" },
        { 177226991, "scarecrow" },
        { 273951840, "scarecrow.suit" },
        { 809942731, "scarecrowhead" },
        { 445662288, "scientist.plushie" },
        { -1772746857, "scientistsuit_heavy" },
        { -932201673, "scrap" },
        { -1094453063, "scrapframe.large" },
        { -1060567807, "scrapframe.medium" },
        { -498301781, "scrapframe.small" },
        { -1774190142, "scrapframe.standing" },
        { -1244287686, "scrapframe.xl" },
        { -1211801774, "scrapframe.xxl" },
        { -82758111, "scrapmirror.large" },
        { 839738457, "scrapmirror.medium" },
        { -1050697733, "scrapmirror.small" },
        { -1380144986, "scrapmirror.standing" },
        { 263834859, "scraptea" },
        { 524678627, "scraptea.advanced" },
        { 2024467711, "scraptea.pure" },
        { 375473148, "scraptransportheli" },
        { 504109620, "sculpture.ice" },
        { 1852905808, "sculpture.rock" },
        { 2087678962, "searchlight" },
        { 567871954, "secretlabchair" },
        { -374457631, "sedan" },
        { 1911552868, "seed.black.berry" },
        { 803954639, "seed.blue.berry" },
        { 998894949, "seed.corn" },
        { -1776128552, "seed.green.berry" },
        { -237809779, "seed.hemp" },
        { 1004843240, "seed.orchid" },
        { -2084071424, "seed.potato" },
        { -1511285251, "seed.pumpkin" },
        { 830839496, "seed.red.berry" },
        { -1037472336, "seed.rose" },
        { 1412103380, "seed.sunflower" },
        { -1790885730, "seed.wheat" },
        { -992286106, "seed.white.berry" },
        { -520133715, "seed.yellow.berry" },
        { 573926264, "semibody" },
        { 1234880403, "sewingkit" },
        { -1994909036, "sheetmetal" },
        { 1950721418, "shelves" },
        { -2025184684, "shirt.collared" },
        { 1608640313, "shirt.tanktop" },
        { -1549739227, "shoes.boots" },
        { -765183617, "shotgun.double" },
        { 678698219, "shotgun.m4" },
        { 795371088, "shotgun.pump" },
        { -41440462, "shotgun.spas12" },
        { -1367281941, "shotgun.waterpipe" },
        { -1536855921, "shovel" },
        { -1199897169, "shutter.metal.embrasure.a" },
        { -1199897172, "shutter.metal.embrasure.b" },
        { -1023374709, "shutter.wood.a" },
        { -1368584029, "sickle" },
        { -1290278434, "siegetower" },
        { -946599114, "sign.artistcanvas.l" },
        { -946599113, "sign.artistcanvas.m" },
        { -946599131, "sign.artistcanvas.s" },
        { 1562867678, "sign.artistcanvas.xl" },
        { 1609921845, "sign.artistcanvas.xs" },
        { -816769770, "sign.artistcanvas.xxl" },
        { 1205607945, "sign.hanging" },
        { 23352662, "sign.hanging.banner.large" },
        { -1647846966, "sign.hanging.ornate" },
        { 1305578813, "sign.neon.125x125" },
        { -1423304443, "sign.neon.125x215" },
        { 42535890, "sign.neon.125x215.animated" },
        { 866332017, "sign.neon.xl" },
        { 1643667218, "sign.neon.xl.animated" },
        { -845557339, "sign.pictureframe.landscape" },
        { -1370759135, "sign.pictureframe.portrait" },
        { 121049755, "sign.pictureframe.tall" },
        { -996185386, "sign.pictureframe.xl" },
        { 98508942, "sign.pictureframe.xxl" },
        { 2070189026, "sign.pole.banner.large" },
        { 1521286012, "sign.post.double" },
        { 1542290441, "sign.post.single" },
        { -1832422579, "sign.post.town" },
        { 826309791, "sign.post.town.roof" },
        { -143132326, "sign.wooden.huge" },
        { 1153652756, "sign.wooden.large" },
        { -1819233322, "sign.wooden.medium" },
        { -1138208076, "sign.wooden.small" },
        { 1849409072, "silly.horse.mask" },
        { -193519904, "single.shallow.wall.shelves" },
        { -1056824343, "skidoo" },
        { 1312843609, "skull" },
        { 996293980, "skull.human" },
        { -769647921, "skull.trophy" },
        { 971362526, "skull.trophy.jar" },
        { -924959988, "skull.trophy.jar2" },
        { -156748077, "skull.trophy.table" },
        { 2048317869, "skull.wolf" },
        { 553887414, "skull_fire_pit" },
        { -216116642, "skulldoorknocker" },
        { -1073015016, "skullspikes" },
        { -25740268, "skullspikes.candles" },
        { -1078639462, "skullspikes.pumpkin" },
        { 1819863051, "skylantern" },
        { -1770889433, "skylantern.skylantern.green" },
        { -1824770114, "skylantern.skylantern.orange" },
        { 831955134, "skylantern.skylantern.purple" },
        { -1433390281, "skylantern.skylantern.red" },
        { -333406828, "sled" },
        { -135252633, "sled.xmas" },
        { -1754948969, "sleepingbag" },
        { -1293296287, "small.oil.refinery" },
        { -158718378, "small_ramp" },
        { 2068884361, "smallbackpack" },
        { -2058362263, "smallcandles" },
        { -2115299615, "smallengine" },
        { -1039528932, "smallwaterbottle" },
        { -695978112, "smart.alarm" },
        { 988652725, "smart.switch" },
        { 1796682209, "smg.2" },
        { 1318558775, "smg.mp5" },
        { -1758372725, "smg.thompson" },
        { 1230323789, "smgbody" },
        { -2100458529, "snakemeat" },
        { -170436364, "snakemeat.cooked" },
        { -1616704051, "snakemeat.spoiled" },
        { -363689972, "snowball" },
        { 1103488722, "snowballgun" },
        { 1358643074, "snowmachine" },
        { 1629293099, "snowman" },
        { -1364246987, "snowmobile" },
        { 1768112091, "snowmobiletomaha" },
        { -555122905, "sofa" },
        { 782422285, "sofa.pattern" },
        { -343857907, "soundlight" },
        { -89874794, "sparkplug1" },
        { -493159321, "sparkplug2" },
        { 1072924620, "sparkplug3" },
        { 695450239, "spear.cny" },
        { 1602646136, "spear.stone" },
        { 1540934679, "spear.wooden" },
        { -1517740219, "speargun" },
        { -1800345240, "speargun.spear" },
        { 963400638, "speechbubbleballoon2025" },
        { 882559853, "spiderweb" },
        { -92759291, "spikes.floor" },
        { -1850297170, "spikes.trap" },
        { -1100422738, "spinner.wheel" },
        { 1184215560, "spoiled.produce" },
        { 1885488976, "spookyspeaker" },
        { -596876839, "spraycan" },
        { -1366326648, "spraycandecal" },
        { -1782127806, "starballoon2025" },
        { -369760990, "stash.small" },
        { -1866909924, "steeringwheel.boat" },
        { 642482233, "sticks" },
        { -465682601, "stocking.large" },
        { 1668858301, "stocking.small" },
        { 171931394, "stone.pickaxe" },
        { -1583967946, "stonehatchet" },
        { -2099697608, "stones" },
        { 1149964039, "storage.monitor" },
        { -258457936, "storage_barrel_a" },
        { 1307626005, "storage_barrel_b" },
        { -1421257350, "storage_barrel_c" },
        { -1049172752, "storageadaptor" },
        { 2104517339, "strobelight" },
        { -1671551935, "submarine.torpedo.straight" },
        { 1015352446, "submarineduo" },
        { -187031121, "submarinesolo" },
        { -1581843485, "sulfur" },
        { -1157596551, "sulfur.ore" },
        { -611118083, "sunflower" },
        { 352321488, "sunglasses" },
        { 1258768145, "sunglasses02black" },
        { -2103694546, "sunglasses02camo" },
        { 1557173737, "sunglasses02red" },
        { -176608084, "sunglasses03black" },
        { -1997698639, "sunglasses03chrome" },
        { -1408336705, "sunglasses03gold" },
        { 789333045, "sunken.knife" },
        { -1003665711, "supertea" },
        { 1397052267, "supply.signal" },
        { 1975934948, "surveycharge" },
        { 1079279582, "syringe.medical" },
        { 2083256995, "t1_smg" },
        { 593465182, "table" },
        { -1108136649, "tactical.gloves" },
        { -1736356576, "target.reactive" },
        { 1523195708, "targeting.computer" },
        { 2019042823, "tarp" },
        { 73681876, "techparts" },
        { 1234878710, "telephone" },
        { 1754952075, "thruster.module" },
        { 962186730, "tincan.alarm" },
        { -1262185308, "tool.binoculars" },
        { -1316706473, "tool.camera" },
        { -2001260025, "tool.instant_camera" },
        { 1803831286, "toolgun" },
        { 795236088, "torch" },
        { -1175656359, "torch.torch.skull" },
        { 446206234, "torchholder" },
        { -582782051, "trap.bear" },
        { -1663759755, "trap.landmine" },
        { 647240052, "triangle.rail.road.planter" },
        { 1991794121, "trike" },
        { 975983052, "trophy" },
        { -901370585, "trophy2023" },
        { 223891266, "tshirt" },
        { 935692442, "tshirt.long" },
        { -561148628, "tugboat" },
        { -1478445584, "tunalight" },
        { -1569700847, "twitch.headset" },
        { -243540612, "twitchrivals2023desk" },
        { 1604092540, "twitchrivals2025sofa" },
        { -739993590, "twitchrivalsflag" },
        { 20489901, "twitchsunglasses" },
        { 1330084809, "valve1" },
        { 926800282, "valve2" },
        { -1802083073, "valve3" },
        { -885833256, "vampire.stake" },
        { -1501451746, "vehicle.1mod.cockpit" },
        { 1874610722, "vehicle.1mod.cockpit.armored" },
        { 170758448, "vehicle.1mod.cockpit.with.engine" },
        { 1559779253, "vehicle.1mod.engine" },
        { -1880231361, "vehicle.1mod.flatbed" },
        { -1615281216, "vehicle.1mod.passengers.armored" },
        { 1376065505, "vehicle.1mod.rear.seats" },
        { 268565518, "vehicle.1mod.storage" },
        { -626174997, "vehicle.1mod.taxi" },
        { -1040518150, "vehicle.2mod.camper" },
        { -1693832478, "vehicle.2mod.flatbed" },
        { 1186655046, "vehicle.2mod.fuel.tank" },
        { 895374329, "vehicle.2mod.passengers" },
        { 721798950, "vehicle.car_radio" },
        { 1770744540, "vehicle.chassis" },
        { -44066600, "vehicle.chassis.2mod" },
        { -44066823, "vehicle.chassis.3mod" },
        { -44066790, "vehicle.chassis.4mod" },
        { 878301596, "vehicle.module" },
        { 198438816, "vending.machine" },
        { -870140677, "venom.snake" },
        { 996757362, "wagon" },
        { -1416322465, "walkietalkie" },
        { 99588025, "wall.external.high" },
        { 756890702, "wall.external.high.adobe" },
        { -985781766, "wall.external.high.ice" },
        { -1993883724, "wall.external.high.legacy" },
        { -967648160, "wall.external.high.stone" },
        { -1429456799, "wall.frame.cell" },
        { -956706906, "wall.frame.cell.gate" },
        { -1117626326, "wall.frame.fence" },
        { 1451568081, "wall.frame.fence.gate" },
        { -148794216, "wall.frame.garagedoor" },
        { 1115193056, "wall.frame.lunar2025_a" },
        { -450890885, "wall.frame.lunar2025_b" },
        { -2016974826, "wall.frame.lunar2025_c" },
        { 1516985844, "wall.frame.netting" },
        { -796583652, "wall.frame.shopfront" },
        { -148229307, "wall.frame.shopfront.metal" },
        { -1679267738, "wall.graveyard.fence" },
        { 1327005675, "wall.ice.wall" },
        { -265202949, "wall.shallow.industrial.shelves" },
        { -819720157, "wall.window.bars.metal" },
        { 671706427, "wall.window.bars.toptier" },
        { -1183726687, "wall.window.bars.wood" },
        { -1614955425, "wall.window.glass.reinforced" },
        { 1730664641, "wallpaper.ceiling" },
        { -551431036, "wallpaper.flooring" },
        { 1629564540, "wallpaper.tool" },
        { 553967074, "wallpaper.wall" },
        { -1344017968, "wantedposter" },
        { 301063058, "wantedposter.wantedposter2" },
        { -1265020883, "wantedposter.wantedposter3" },
        { 1463862472, "wantedposter.wantedposter4" },
        { -1142222427, "warmingtea" },
        { -463122489, "watchtower.wood" },
        { -1779180711, "water" },
        { -1863559151, "water.barrel" },
        { -1100168350, "water.catcher.large" },
        { -132247350, "water.catcher.small" },
        { 2114754781, "water.purifier" },
        { 1811780502, "water.radioactive" },
        { -277057363, "water.salt" },
        { -119235651, "waterjug" },
        { -1284169891, "waterpump" },
        { 174866732, "weapon.mod.8x.scope" },
        { 838308300, "weapon.mod.burstmodule" },
        { 2005491391, "weapon.mod.extendedmags" },
        { 952603248, "weapon.mod.flashlight" },
        { -1767794021, "weapon.mod.gascompressionovedrive" },
        { 442289265, "weapon.mod.holosight" },
        { -132516482, "weapon.mod.lasersight" },
        { -1405508498, "weapon.mod.muzzleboost" },
        { 1478091698, "weapon.mod.muzzlebrake" },
        { -781866273, "weapon.mod.oilfiltersilencer" },
        { -1850571427, "weapon.mod.silencer" },
        { -855748505, "weapon.mod.simplesight" },
        { 567235583, "weapon.mod.small.scope" },
        { -1659598760, "weapon.mod.sodacansilencer" },
        { 1719587208, "weapon.mod.targetingattachment" },
        { 1277159544, "weaponrack.doublelight" },
        { -1163943815, "weaponrack.light" },
        { 1178325727, "wheat" },
        { 854447607, "white.berry" },
        { 22947882, "whiteidtag" },
        { -526026171, "wicker.barrel" },
        { 738611016, "window.paintable" },
        { -144417939, "wiretool" },
        { 1827479659, "wolfmeat.burned" },
        { 813023040, "wolfmeat.cooked" },
        { -395377963, "wolfmeat.raw" },
        { -1167031859, "wolfmeat.spoiled" },
        { -151838493, "wood" },
        { -2094954543, "wood.armor.helmet" },
        { 418081930, "wood.armor.jacket" },
        { 832133926, "wood.armor.pants" },
        { -459159118, "woodarmor.gloves" },
        { 699075597, "woodcross" },
        { 1604837581, "wooden.shield" },
        { -635951327, "woodframe.large" },
        { -1541706279, "woodframe.medium" },
        { -1476278729, "woodframe.small" },
        { 1769475390, "woodframe.standing" },
        { 1312679249, "woodmirror.large" },
        { 756125481, "woodmirror.medium" },
        { -1497205569, "woodmirror.small" },
        { 723407026, "woodmirror.standing" },
        { -649128577, "woodtea" },
        { -541206665, "woodtea.advanced" },
        { -557539629, "woodtea.pure" },
        { 798382300, "workbench.upgrade.accelerated" },
        { -770390391, "workbench.upgrade.comfort" },
        { -1953279770, "workbench.upgrade.defensive" },
        { 1215602244, "workbench.upgrade.efficiency" },
        { -180862419, "workbench.upgrade.prototype" },
        { 1470387662, "workbench.upgrade.range" },
        { -286541059, "workbench.upgrade.recyclebin" },
        { 112268546, "workbench.upgrade.reinforced" },
        { -160105346, "workbench.upgrade.salvage" },
        { -1536343135, "workbench.upgrade.surplus" },
        { 1524187186, "workbench1" },
        { -41896755, "workbench2" },
        { -1607980696, "workbench3" },
        { -810326667, "workcart" },
        { 1770475779, "worm" },
        { 204970153, "wrappedgift" },
        { 1094293920, "wrappingpaper" },
        { -2027793839, "xmas.advent" },
        { -1667224349, "xmas.decoration.baubels" },
        { -209869746, "xmas.decoration.candycanes" },
        { 1686524871, "xmas.decoration.gingerbreadmen" },
        { 1723747470, "xmas.decoration.lights" },
        { -129230242, "xmas.decoration.pinecone" },
        { -1331212963, "xmas.decoration.star" },
        { 2106561762, "xmas.decoration.tinsel" },
        { 674734128, "xmas.door.garland" },
        { -1230433643, "xmas.double.door.garland" },
        { 1058261682, "xmas.lightstring" },
        { -151387974, "xmas.lightstring.advanced" },
        { -1622660759, "xmas.present.large" },
        { 756517185, "xmas.present.medium" },
        { -722241321, "xmas.present.small" },
        { 794443127, "xmas.tree" },
        { -1379835144, "xmas.window.garland" },
        { 2009734114, "xmasdoorwreath" },
        { -211235948, "xylophone" },
        { 1660145984, "yellow.berry" },
        { 81423963, "yellowidtag" },
    }};

    inline const std::unordered_map<std::int32_t, std::string_view>
        ITEM_ID_TO_SHORTNAME = []
        {
            std::unordered_map<std::int32_t, std::string_view> result;
            result.reserve(ITEMS.size());

            for (const ItemEntry& item : ITEMS)
                result.emplace(item.item_id, item.shortname);

            return result;
        }();

    inline const std::unordered_map<std::string_view, std::int32_t>
        SHORTNAME_TO_ITEM_ID = []
        {
            std::unordered_map<std::string_view, std::int32_t> result;
            result.reserve(ITEMS.size());

            for (const ItemEntry& item : ITEMS)
                result.emplace(item.shortname, item.item_id);

            return result;
        }();

    inline bool try_get_shortname(
        std::int32_t item_id,
        std::string_view& shortname) noexcept
    {
        const auto it = ITEM_ID_TO_SHORTNAME.find(item_id);
        if (it == ITEM_ID_TO_SHORTNAME.end())
            return false;

        shortname = it->second;
        return true;
    }

    inline bool try_get_item_id(
        std::string_view shortname,
        std::int32_t& item_id) noexcept
    {
        const auto it = SHORTNAME_TO_ITEM_ID.find(shortname);
        if (it == SHORTNAME_TO_ITEM_ID.end())
            return false;

        item_id = it->second;
        return true;
    }

    inline std::string_view get_shortname(std::int32_t item_id) noexcept
    {
        const auto it = ITEM_ID_TO_SHORTNAME.find(item_id);
        return it != ITEM_ID_TO_SHORTNAME.end()
            ? it->second
            : std::string_view{};
    }

    inline std::int32_t get_item_id(std::string_view shortname) noexcept
    {
        const auto it = SHORTNAME_TO_ITEM_ID.find(shortname);
        return it != SHORTNAME_TO_ITEM_ID.end() ? it->second : 0;
    }
}
